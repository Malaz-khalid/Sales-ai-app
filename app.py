...
def generate_report(prediction, quantity, discount, profit, category, segment):

    file_name = "sales_report.pdf"
    c = canvas.Canvas(file_name)

    c.drawString(100, 750, "AI Sales Prediction Report")
    c.drawString(100, 720, f"Prediction: {prediction:.2f}")
    c.drawString(100, 700, f"Quantity: {quantity}")
    c.drawString(100, 680, f"Discount: {discount}")
    c.drawString(100, 660, f"Profit: {profit}")
    c.drawString(100, 640, f"Category: {category}")
    c.drawString(100, 620, f"Segment: {segment}")
