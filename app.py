...
if st.button("🚀 Predict"):
    ...
    st.success(f"💰 Expected Sales: {lower:.2f} - {upper:.2f}")

    # ---------------- BUSINESS SCORE ----------------
    if prediction > 300:
        st.success("🔥 High Sales Potential")
    elif prediction > 150:
        st.info("📊 Medium Sales Potential")
    else:
        st.warning("⚠️ Low Sales Potential")
    ...
    if discount > 0.5:
        summary.append("High discount strategy used.")
    if quantity > 50:
        summary.append("High demand product.")
    if profit > 50:
        summary.append("High profitability product.")
